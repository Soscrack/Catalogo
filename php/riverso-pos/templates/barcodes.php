        ...
        $.post(ajaxurl, {
            ...
            if (response.success) {
                ...
                if (typeof RiversoLabelPrint !== 'undefined') {
                    html += `<button type="button" class="button button-small print-label-btn" 
                        data-sku="${p.sku}" 
                        data-nombre="${p.name}"
    ...
    function loadBarcodes() {
        $.post(ajaxurl, {
            ...
            response.data.barcodes.forEach(function(b) {
                const printBtn = b.sku && typeof RiversoLabelPrint !== 'undefined' 
                    ? `<button class="button button-small print-label-btn" 
                        data-sku="${b.sku}" 
                        data-nombre="${b.product_name || b.sku}"
                        data-precio="${b.price || 0}"
