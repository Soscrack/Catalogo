...
<div class="wrap">
    ...
    <table class="wp-list-table widefat fixed striped">
        <thead><tr><th>ID</th><th>SKU bolsa</th><th>Cantidad</th><th>EAN13</th><th>Costo</th><th>Estado</th><th></th></tr></thead>
        <tbody id="pk-bolsas-tbody"><tr><td colspan="7">-</td></tr></tbody>
    </table>
</div>
...
jQuery(function($){
    ...
    function loadBolsas(){
        ...
        $.post(ajaxurl, {action:'riverso_packaging_bolsas', nonce, producto_base_id:baseId()}, function(r){
            ...
            const rows = (r.data.bolsas||[]).map(b => {
                const printBtn = b.sku_bolsa && typeof RiversoLabelPrint !== 'undefined'
                    ? `<button class="button button-small print-label-btn" 
                        data-sku="${b.sku_bolsa}" 
                        data-nombre="Bolsa ${b.sku_bolsa}"
                        data-ean13="${b.ean13}"
                        data-precio="0"
                        title="Imprimir etiqueta de bolsa">
                        🖨️
                    </button>`
                    : '';
                return `
    ...
    $('#pk-bolsa-create').on('click', function(){
        ...
        $.post(ajaxurl, {action:'riverso_packaging_create_bolsa', nonce, producto_base_id:baseId(), cantidad:$('#pk-bolsa-qty').val()}, function(r){
            ...
            // Mostrar resultado con botón de impresión
            let result = ' EAN13: <strong>' + (r.data.ean13 || 'sin EAN') + '</strong>';
            if (r.data.sku_bolsa && typeof RiversoLabelPrint !== 'undefined') {
                result += ` <button class="button button-small print-label-btn" 
                    data-sku="${r.data.sku_bolsa}" 
                    data-nombre="Bolsa ${r.data.sku_bolsa}"
                    data-ean13="${r.data.ean13}"
                    data-precio="0"
                    style="margin-left:8px;">
                    🖨️ Imprimir
                </button>`;
            }
            $('#pk-bolsa-result').html(result);
            reloadAll();
        });
    });
