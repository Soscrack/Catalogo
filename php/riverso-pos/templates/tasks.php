...
jQuery(function($) {
    ...
    function renderTasks(tasks) {
        ...
        tasks.forEach(function(task) {
            // Botón de impresión para tareas de etiquetado
            let printBtn = '';
            if ((task.tipo === 'etiquetado' || task.tipo === 'verificar_etiquetado') && 
                typeof RiversoLabelPrint !== 'undefined' && 
                task.datos_extra && task.datos_extra.items && task.datos_extra.items.length > 0) {
                printBtn = `<button class="button button-small btn-print-task-batch" 
    ...
    // Imprimir lote de etiquetas desde tarea
    $(document).on('click', '.btn-print-task-batch', function(e) {
        e.preventDefault();
        ...
        if (!task.datos_extra || !task.datos_extra.items || !task.datos_extra.items.length) {
            alert('No hay ítems para imprimir en esta tarea');
            return;
        }

        // Construir batch de trabajos desde los items de la tarea
        ...
        if (jobs.length === 1) {
            // Si es solo un ítem, usar diálogo modal
            RiversoLabelPrint.showPrintDialog(jobs[0]);
        } else {
            // Si es lote, confirmar e imprimir directamente
            if (confirm(`¿Imprimir ${jobs.length} etiquetas del lote?`)) {
