#!/usr/bin/env python3
"""Instala el cron de precios DIMAFI a las 02:00 America/Santiago.

Plesk programa en UTC del servidor, así que `0 2 * * *` en la tarea Plesk
...
SCRIPT = (
    ...
    "riverso-pos/cli/dimafi-precios-refresh.php"
)
LOG_DIR = "/var/www/vhosts/riverso.cl/httpdocs/wp-content/uploads/riverso-logs"
CRON_FILE = "/etc/cron.d/riverso-dimafi-precios"
USER = "riverso.cl_1xybiw6rlcq"
DESC = "DIMAFI precios 02:00 America/Santiago"
CRON_BODY = f"""# Precios DIMAFI: 02:00 America/Santiago (respeta horario de verano)
SHELL=/bin/bash
...
def find_task(rows):
    for row in rows:
        blob = json.dumps(row)
        if "dimafi-precios" in blob or DESC in blob or "dimafi-precios-refresh" in blob:
