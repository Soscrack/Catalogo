#!/usr/bin/env python3
"""
Importa JSONL de competencia DIMAFI a la BD remota (staging riverso_competencia_*).

Requiere haber ejecutado antes:
  python tools/scrape_dimafi_catalog.py

Uso:
  python tools/import_competencia_dimafi.py
    python tools/import_competencia_dimafi.py --dry-run
    python tools/import_competencia_dimafi.py --skip-media
"""
...
DEFAULT_DATA = ROOT / "data" / "competencia" / "dimafi"
FUENTE_SLUG = "dimafi"
...
def build_sql(datadir: Path, rprefix: str) -> str:
    ...
    lines.append(
        ...
        f"VALUES ('{FUENTE_SLUG}', 'DIMAFI', 'https://www.dimafi.cl', 1) "
        "ON DUPLICATE KEY UPDATE nombre=VALUES(nombre), base_url=VALUES(base_url), activo=1;"
    )
    lines.append("SET @fuente_id = (SELECT id FROM `{p}competencia_fuentes` WHERE slug = '{s}' LIMIT 1);".format(p=rprefix, s=FUENTE_SLUG))
    ...
    # Productos (incluye id_grupo_externo + descripcion).
    for p in productos:
        ext_id = p.get("id_externo")
        lines.append(
            f"INSERT INTO `{rprefix}competencia_productos` "
            "(fuente_id, id_externo, id_grupo_externo, descripcion, codigo_externo, codigo_normalizado, nombre, slug, url_producto, "
            "marca, id_marca, fabricante, id_division, id_seccion, id_categoria, nombre_division, nombre_seccion, nombre_categoria, "
            "unidad_min_venta, tipo_unidad, peso, stock, situacion, imagen_principal, capturado_at) "
            f"SELECT @fuente_id, {sql_val(ext_id)}, {sql_val(p.get('id_grupo_externo'))}, {sql_val(p.get('descripcion'))}, "
            f"{sql_val(p.get('codigo_externo'))}, {sql_val(p.get('codigo_normalizado'))}, {sql_val(p.get('nombre'))}, "
            f"{sql_val(p.get('slug'))}, {sql_val(p.get('url_producto'))}, "
            ...
            f"{sql_val(p.get('nombre_division'))}, {sql_val(p.get('nombre_seccion'))}, {sql_val(p.get('nombre_categoria'))}, "
            f"{sql_val(p.get('unidad_min_venta'))}, {sql_val(p.get('tipo_unidad'))}, {sql_val(p.get('peso'))}, {sql_val(p.get('stock'))}, "
            f"{sql_val(p.get('situacion'))}, {sql_val(p.get('imagen_principal'))}, {sql_val(p.get('capturado_at'))} "
            "ON DUPLICATE KEY UPDATE "
            "id_grupo_externo=VALUES(id_grupo_externo), descripcion=VALUES(descripcion), "
            ...
            "tipo_unidad=VALUES(tipo_unidad), peso=VALUES(peso), stock=VALUES(stock), "
            "situacion=VALUES(situacion), imagen_principal=VALUES(imagen_principal), capturado_at=VALUES(capturado_at), "
            "updated_at=CURRENT_TIMESTAMP;"
        )

    # Precios: una fila por producto (vigente).
    ...
    for ext_id, pr in precio_by_ext.items():
        lines.append(
            ...
            f"SELECT cp.id, {sql_val(pr.get('snapshot_fecha'))}, {sql_val(pr.get('precio'))}, {sql_val(pr.get('precio_lista'))}, "
            f"{sql_val(pr.get('precio_bruto_unitario'))}, {sql_val(pr.get('precio_bruto_total'))}, {sql_val(pr.get('cantidad_min'))}, "
            f"{sql_val(pr.get('iva'))}, {sql_val(pr.get('costo'))}, {sql_val(pr.get('moneda') or 'CLP')}, {sql_val(pr.get('oculto'))}, NOW() "
            f"FROM `{rprefix}competencia_productos` cp "
            f"INNER JOIN `{rprefix}competencia_fuentes` f ON f.id = cp.fuente_id AND f.slug = '{FUENTE_SLUG}' "
    ...
    # Medios (fotos + pdf/ficha).
    for m in medios:
        ruta = m.get("ruta_local") or ""
        if ruta and not str(ruta).startswith("riverso-competencia/"):
            # Mismo normalizador que import_competencia_sande.py.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import competencia DIMAFI a BD remota")
    ...
    if not manifest.is_file():
        print(f"No existe {manifest}. Ejecuta scrape_dimafi_catalog.py primero.")
        return 1
    ...
    if args.dry_run:
        print(f"SQL generado: {len(sql_body)} bytes (dry-run, no se sube)", flush=True)
        ssh.close()
        return 0

    if not args.skip_media:
        n = upload_media(ssh, datadir, wp_path)
        print(f"Medios subidos: {n}", flush=True)

    with tempfile.NamedTemporaryFile("w", suffix=".sql", delete=False, encoding="utf-8") as tmp:
        tmp.write(sql_body)
        local_sql = tmp.name

    remote_sql = "/tmp/competencia_dimafi_import.sql"
    sftp = ssh.open_sftp()
    print("Subiendo SQL...", flush=True)
    ...
    print("Ejecutando import MySQL...", flush=True)
    ...
    count_cmd = (
        ...
        f"INNER JOIN `{rprefix}competencia_fuentes` f ON f.id=p.fuente_id WHERE f.slug='{FUENTE_SLUG}';\""
    )
    _, count, _ = run_ssh(ssh, count_cmd, timeout=30)
    print(f"Productos competencia {FUENTE_SLUG} en BD: {count.strip()}", flush=True)
    ssh.close()
